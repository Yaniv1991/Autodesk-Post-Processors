/**
  ...
  $Revision: 42892 8ce4709df5b8d6a693ecdce003b4ed6b2076b4c3 $
...
propertyDefinitions = {
  ...
  forceXyMode: {title:"Force XY Milling",description:"Forces use of XY (Use when an axial milling crosses the centerline or when X is lower than 10mm in diameter", type:"boolean"},
...
function onOpen() {
  ...
  writeBlock(gFormat.format(50), sOutput.format(getSection(0).spindle == SPINDLE_PRIMARY ? properties.maximumSpindleSpeed : properties.subMaximumSpindleSpeed));
  sOutput.reset();
  ...
  if (properties.gotSecondarySpindle) {
    writeBlock(gFormat.format(53), gMotionModal.format(0), "B" + spatialFormat.format(properties.g53WorkPositionSub)); // retract Sub Spindle if applicable
  }
...
*/
  ...
  if(machineState.forceXyMillingMode){
    setForcedXyMillingMode(true);
  }
  gMotionModal.reset();
...
function updateMachiningMode(section) {
  ...
  machineState.forceXyMillingMode = false; //reset
        ...
        if (bestABCIndex != undefined) {
          ...
          if (properties.useG112 || forcePolarMode) {
            ...
            machineState.usePolarMode = true;
              ...
              if(properties.forceXyMode){
                    machineState.forceXyMillingMode=true;
              }
              else{
                  machineState.useXZCMode = true;
              }
...
function setForcedXyMillingMode(activate) {
  if (activate) {
    writeBlock(gMotionModal.format(0), cOutput.format(0)); // set C-axis to 0 to avoid G112 issues
    ...
    xFormat.setScale(2); // diameter mode
    xOutput = createVariable({prefix:"X"}, xFormat);
    yOutput.enable(); // required for XY Milling
  } else {
    writeBlock(getCode("POLAR_INTERPOLATION_OFF"));
    xFormat.setScale(2); // diameter mode
    xOutput = createVariable({prefix:"X"}, xFormat);
...
properties.maximumSpindleSpeed = 4000;
properties.subMaximumSpindleSpeed = 4000;
