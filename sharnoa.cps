  /**
Taken from a FANUC Post Processor with this legal notice:
...
longDescription = "Generic post for Sharnoa.";
...
propertyDefinitions = {
  //machineModel: {title:"Machine model" ,description:"M44 or LU400?" ,type:"enum",values:[{title:"M44" ,id: "M44"}, {title: "LU400", id:"LU400"}] },
...
var permittedCommentChars = " abcdefghijklnmopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,=_-";

var gFormat = createFormat({prefix:"G", width:2, zeropad:false, decimals:1});
...
function onOpen() {
  ...
  if (true) { // note: setup your machine here
    var bAxis = createAxis({coordinate:1, table:true, axis:[0, 1, 0], range:[-40, 109.1], preference:1});
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    if (properties.preloadTool) {
      ...
      if (nextTool) {
        writeBlock("M14 T" + toolFormat.format(nextTool.number));
      } else {
        // preload first tool
        var section = getSection(0);
        var firstToolNumber = section.getTool().number;
        if (tool.number != firstToolNumber) {
          writeBlock("M14 T" + toolFormat.format(firstToolNumber));
        }
      }
    }
  }
  ...
  if (workOffset > 0) {
    if (workOffset > 5) {
      ...
      if (workOffset != currentWorkOffset) {
        writeBlock(gFormat.format(53 + workOffset)); // G54->G58
...
//TODO Pretty much redo all this shit... For now, i'm just disabling canned cycles.
...
// TODO Write this retract too

function writeRetract() {
   if(properties.machineModel == "M44"){
    writeBlock(gFormat.format(116), "Z-10.5 Y-10.0");
  }

  if(properties.machineModel == "LU400"){
    writeBlock(gFormat.format(116), "Z480.0");
    writeBlock(gFormat.format(116), "X-250.0 Y160.0");
...
{/** Output block to do safe retract and/or move to home position. */
 // initialize routine
  ...
  //     gMotionModal.reset();
  //     writeBlock(gAbsIncModal.format(90), gFormat.format(116), gMotionModal.format(0), words);
